...
typedef struct {
    ...
    union {
        struct {
            uint16_t Byte : 8;  // bit [ 7: 0] Byte in command that contained
                                // the error.
            uint16_t Bit : 3;   // bit [10: 8] Bit in command that contained the
                                // error.
    ...
    uint8_t VendorInfoAvailable;  // byte [   28] Vendor Specific Information
                                  // Available
    ...
    uint16_t TRTypeSpecificInfo;   // byte [41:40] Transport Type Specific
                                   // Information <v1.4>
    uint8_t Reserved1[22];         // byte [63:42]
} NVME_ERROR_INFO_LOG14, *PNVME_ERROR_INFO_LOG14;

static void s_vPrintNVMeErrorInformation(PNVME_ERROR_INFO_LOG14 _pData,
                                         int _iLogNo) {
    ...
    if (bIsNVMeV14OrLater()) {
        printf(
            "\tbyte [ 41: 40] 0x%04X = Transport Type Specific Information\n ",
    ...
    if (bIsNVMeV14OrLater()) {
        ...
        if (_pData->TRTYPE == 0) {
            printf(
                "00h = The transport type is not indicated or the error is not "
                "transport related\n");
        } else if (_pData->TRTYPE == 1) {
            printf(
                "01h = RDMA Transport (refer to the NVMe over Fabric "
                "specification)\n");
        } else if (_pData->TRTYPE == 2) {
            printf("02h = Fibre Channel Transport (refer to INCITS 540)\n");
        } else if (_pData->TRTYPE == 3) {
            printf(
                "03h = TCP Transport (refer to the NVMe over Fabrics "
                "specification)\n");
    ...
    if (_pData->VendorInfoAvailable) {
        printf(
            "\tbyte [     28] %02Xh = Log Page ID for additional vendor "
            "specific error information\n",
            _pData->VendorInfoAvailable);
    } else {
        printf(
            "\tbyte [     28] 00h = No additional information is available\n");
    }

    if (_pData->NameSpace) {
        printf(
            "\tbyte [ 27: 24] %Xh = The namespace id that the error is "
            "associated with\n",
            _pData->NameSpace);
    } else {
    ...
    printf(
        "\tbyte [ 23: 16] %llXh = The first LBA that experienced the error "
        "condition, if applicable\n",
        _pData->Lba);
    printf("\tbyte [ 15: 14] Parameter Error Location:\n");
    if (_pData->ParameterErrorLocation.AsUshort == 0xFFFF) {
        printf(
            "\t\tbit [ 15:  0] FFFFh = The error is not specific to a "
            "particular command\n");
    } else {
        printf(
            "\t\tbit [ 10:  8] %Xh = Bit in command that contained the error\n",
            _pData->ParameterErrorLocation.Bit);
        printf(
            "\t\tbit [  7:  0] %Xh = Byte in command that contained the "
            "error\n",
...
int iNVMeGetErrorInformation(HANDLE _hDevice) {
    ...
    if ((protocolDataDescr->Version !=
        ...
        fprintf(
            stderr,
            "[E] NVMeGetErrorInformation: Data descriptor header not valid.\n");
        iResult = -1;  // error
        goto error_exit;
    }
    ...
    if ((protocolData->ProtocolDataOffset <
        ...
        (protocolData->ProtocolDataLength <
         sizeof(NVME_ERROR_INFO_LOG14) * (g_stController.ELPE + 1))) {
        fprintf(stderr,
                "[E] NVMeGetErrorInformation: ProtocolData Offset/Length not "
                "valid.\n");
        iResult = -1;  // error
        goto error_exit;
    }
    ...
    {
        PNVME_ERROR_INFO_LOG14 aLog =
            (PNVME_ERROR_INFO_LOG14)((PCHAR)protocolData +
