...
int iNVMeSetFeaturesHCTM(HANDLE _hDevice) {
    ...
    if (g_stController.HCTMA.Supported == 0) {
        fprintf(stderr,
                "\n[E] This SSD controller does not support Host Controlled "
                "Thermal Management (HCTM), skip\n");
        return iResult;
    }

    printf("\n[I] Minimum Thermal Management Temperature (MNTMT) is %d (K)",
           g_stController.MNTMT);
    printf("\n[I] Maximum Thermal Management Temperature (MXTMT) is %d (K)",
           g_stController.MXTMT);
    printf(
        "\n[I] You can speficy TMT1 and TMT2, satisfying TMT1 < TMT2, MNTMT <= "
        "TMT1, and TMT2 <= MXTMT\n");

    {
        ...
        iTMT1 = iGetConsoleInputDec(
            "\n# Input Thermal Management Temperature 1 (TMT1) in Kelvin "
            "(lower temperature)\n",
            strCmd);
        iTMT2 = iGetConsoleInputDec(
            "\n# Input Thermal Management Temperature 2 (TMT2) in Kelvin "
            "(higher temperature)\n",
            strCmd);

        sprintf_s(strPrompt, 1024,
                  "\n# You specified TMT1 = %d (K) and TMT2 = %d (K), press "
                  "'y' to continue\n",
                  iTMT1, iTMT2);
    ...
    if (!(iTMT1 < iTMT2)) {
        fprintf(stderr,
                "\n[E] Should be TMT1 < TMT2, you tried TMT1 = %d and TMT2 = "
                "%d, process aborted\n",
                iTMT1, iTMT2);
        return iResult;
    }
    ...
    if (!(g_stController.MNTMT <= iTMT1)) {
        fprintf(stderr,
                "\n[E] Should be MNTMT <= TMT1, you tried TMT1 = %d against "
                "MNTMT = %d, process aborted\n",
    ...
    if (!(iTMT2 <= g_stController.MXTMT)) {
        fprintf(stderr,
                "\n[E] Should be TMT2 <= MXTMT, you tried TMT2 = %d against "
                "MXTMT = %d, process aborted\n",
