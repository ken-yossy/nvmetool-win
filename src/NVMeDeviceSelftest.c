...
static int siNVMeDST(HANDLE _hDevice, int _iAction) {
    ...
    if (buffer != NULL) {
        ...
        pNvmeCmd->u.GENERAL.CDW10 =
            _iAction;  // 1h: start short test, 2h: start extended test, Fh:
                       // abort test

        // Send request down.
        iResult = iIssueDeviceIoControl(
            _hDevice, IOCTL_STORAGE_PROTOCOL_COMMAND, buffer, bufferLength,
            buffer, bufferLength, &returnedLength, NULL);
        printf("\n");
    } else {
...
int iNVMeDeviceSelftest(HANDLE _hDevice) {
    ...
    sprintf_s(strPrompt, 1024,
              ...
              "\n",
              DST_CODE_SHORT_TEST, DST_CODE_EXTENDED_TEST, DST_CODE_ABORT_TEST);
    ...
    switch (iStc) {
        case DST_CODE_SHORT_TEST:
            if (bInProgress) {
                fprintf(stderr,
                        "[E] Device Self-test - Another test is in progress, "
                        "skip operation\n");
            } else {
                cCmd = cGetConsoleInput(
                    "\n# Device Self-test - Start a short test, Press 'y' to "
                    "continue\n",
                    strCmd);
                if (cCmd == 'y') {
        ...
        case DST_CODE_EXTENDED_TEST:
            if (bInProgress) {
                ...
                printf(
                    "#\tExtended device self-test takes about %d minutes "
                    "according to the Identify data\n",
                    g_stController.EDSTT);
                cCmd = cGetConsoleInput(
                    "\n#\tIf you want to start an extended test, Press 'y' to "
                    "continue\n",
                    strCmd);
                if (cCmd == 'y') {
                    iResult = siNVMeDST(_hDevice, iStc);
        ...
        case DST_CODE_ABORT_TEST:
            if (bInProgress) {
                cCmd = cGetConsoleInput(
                    "\n# Device Self-test - Abort test, Press 'y' to "
                    "continue\n",
                    strCmd);
                if (cCmd == 'y') {
                    iResult = siNVMeDST(_hDevice, iStc);
                ...
                fprintf(stderr,
                        "[E] Device Self-test - No test is in progress, skip "
                        "operation\n");
            }
            break;
