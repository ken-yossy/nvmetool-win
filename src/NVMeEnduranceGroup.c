...
static void s_vPrintNVMeEnduranceGroupInformation(
    PNVME_ENDURANCE_GROUP_INFO _pData) {
    ...
    if (_pData->CriticalWarning.NamespaceIsReadOnly == 1) {
        printf(
            "\tbit [    3] 0x%02X = All namespaces in this endurance group are "
            "placed in read only mode\n",
            _pData->CriticalWarning.NamespaceIsReadOnly);
    } else {
        printf(
            "\tbit [    3] 0x%02X = No namespace in this endurance group is "
            "placed in read only mode\n",
            _pData->CriticalWarning.NamespaceIsReadOnly);
    }
    ...
    if (_pData->CriticalWarning.ReliabilityDegraded == 1) {
        printf(
            "\tbit [    2] 0x%02X = Reliability of this endurance group is "
            "degraded\n",
            _pData->CriticalWarning.ReliabilityDegraded);
    } else {
        printf(
            "\tbit [    2] 0x%02X = Reliability of this endurance group is not "
            "degraded\n",
            _pData->CriticalWarning.ReliabilityDegraded);
    }
    ...
    if (_pData->CriticalWarning.SpareCapacityIsBelowThreshold == 1) {
        printf(
            "\tbit [    0] 0x%02X = Available spare capacity of this endurance "
            "group is fallen below the threshold\n",
            _pData->CriticalWarning.SpareCapacityIsBelowThreshold);
    } else {
        printf(
            "\tbit [    0] 0x%02X = Available spare capacity of this endurance "
            "group is above the threshold\n",
            _pData->CriticalWarning.SpareCapacityIsBelowThreshold);
    }
    printf("\n");
    ...
    printf("byte [143:128] 0x%08llX%08llX = Media and Data Integrity Errors\n",
           _pData->MediaAndDataIntegrityErrorsH,
           _pData->MediaAndDataIntegrityErrorsL);
    printf(
        "byte [159:144] 0x%08llX%08llX = Number of Error Information Entries\n",
        _pData->NumferOfErrorInformationLogEntriesH,
        _pData->NumberOfErrorInformationLogEntriesL);
}
...
int iNVMeGetEnduranceGroupInformation(HANDLE _hDevice) {
    ...
    if ((protocolDataDescr->Version !=
        ...
        fprintf(stderr,
                "[E] NVMeGetEnduranceGroupInformation: Data descriptor header "
                "not valid.\n");
        iResult = -1;  // error
        goto error_exit;
    }
    ...
    if ((protocolData->ProtocolDataOffset <
        ...
        (protocolData->ProtocolDataLength <
         sizeof(NVME_ENDURANCE_GROUP_INFO))) {
        fprintf(stderr,
                "[E] NVMeGetEnduranceGroupInformation: ProtocolData "
    ...
    {
        PNVME_ENDURANCE_GROUP_INFO aLog =
            (PNVME_ENDURANCE_GROUP_INFO)((uint8_t *)protocolData +
...
#define NUM_MAX_ENDURANCE_GROUP_ID \
    (252)  // FIXME!!! to make the size of structure below 512 bytes

typedef struct {
    uint64_t m_ullNumEntries;
    uint16_t m_ausEntry[NUM_MAX_ENDURANCE_GROUP_ID];
} NVME_ENDURANCE_GROUP_EVENT_AGGREGATE_LOG,
    *PNVME_ENDURANCE_GROUP_EVENT_AGGREGATE_LOG;
...
int iNVMeGetEnduranceGroupEventAggregateLogPage(HANDLE _hDevice) {
    ...
    protocolData->ProtocolDataRequestValue =
        NVME_LOG_PAGE_ENDURANCE_GROUP_EVENT_AGGREGATE;  // 0Fh
    ...
    PNVME_ENDURANCE_GROUP_EVENT_AGGREGATE_LOG pData =
        (PNVME_ENDURANCE_GROUP_EVENT_AGGREGATE_LOG)((PCHAR)protocolData +
                                                    protocolData
                                                        ->ProtocolDataOffset);
    ...
    if (pData->m_ullNumEntries != 0) {
        if (NUM_MAX_ENDURANCE_GROUP_ID < pData->m_ullNumEntries) {
            printf(
                "[W] This drive has %llu valid entries, but this tool only "
                "show %d entries\n",
                pData->m_ullNumEntries, NUM_MAX_ENDURANCE_GROUP_ID);
        }

        for (int i = 0;
             (i < pData->m_ullNumEntries) && (i < NUM_MAX_ENDURANCE_GROUP_ID);
             i++) {
