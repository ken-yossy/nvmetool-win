...
bool bIsNVMeV20OrLater(void) {
    if (0x00020000 <= ((g_stController.VER) & 0xFFFFFF00)) return true;

    return false;
}

bool bIsNVMeV14OrLater(void) {
    if (0x00010400 <= ((g_stController.VER) & 0xFFFFFF00)) return true;

    return false;
}

bool bIsNVMeV13OrLater(void) {
    if (0x00010300 <= ((g_stController.VER) & 0xFFFFFF00)) return true;
...
void vPrintControllerBasicData(void) {
    ...
    printf(" (NVMe Revision %d.%d.%d)\n", (g_stController.VER >> 16) & 0xFFFF,
           (g_stController.VER >> 8) & 0xFF, g_stController.VER & 0xFF);
    ...
    if ((strstr((const char*)strMN, "WDC WDS") == NULL) ||
        ...
        /**
         * It is unable to get controller's SMART / Health information log page
         * by Get Log Page command with NSID = FFFFFFFFh. So, NSID = 0 is used
         * for a workaround.
         */
        g_WA_bGetControllerSMARTLogWithNSIDZero = true;
        fprintf(stderr,
                "[W] Workanound is enabled: WD SN550 : Cannot retrieve "
                "controller's SMART/Health information by Get Log Page command "
                "with NSID FFFFFFFFh\n");
    }
}
...
int iNVMeIdentifyController(HANDLE _hDevice) {
    ...
    if ((protocolDataDescr->Version !=
        ...
        fprintf(stderr,
                "[E] NVMeIdentifyController: Data Descriptor Header is not "
                "valid, stop.\n");
        iResult = -1;  // error
        goto error_exit;
    }
    ...
    if ((protocolData->ProtocolDataOffset >
        ...
        (protocolData->ProtocolDataLength <
         sizeof(NVME_IDENTIFY_CONTROLLER_DATA))) {
        fprintf(stderr,
                "[E] NVMeIdentifyController: ProtocolData Offset/Length is not "
                "valid, stop.\n");
        iResult = -1;  // error
        goto error_exit;
    }

    memcpy_s((void*)(&g_stController), sizeof(NVME_IDENTIFY_CONTROLLER_DATA),
             (uint8_t*)protocolData + protocolData->ProtocolDataOffset,
             sizeof(NVME_IDENTIFY_CONTROLLER_DATA));
    if ((g_stController.VID == 0) || (g_stController.NN == 0)) {
        fprintf(stderr,
                "[E] NVMeIdentifyController: Identify Controller Data is not "
                "valid, stop.\n");
        iResult = -1;  // error
        goto error_exit;
    }
