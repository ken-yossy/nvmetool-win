...
static int siNVMeGetFeaturesTimestamp(HANDLE _hDevice,
                                      NVME_CDW10_GET_FEATURES _cdw10,
                                      uint64_t *_pullResult) {
    int iResult = -1;
    PVOID buffer = NULL;
    ...
    bufferLength = offsetof(STORAGE_PROPERTY_QUERY, AdditionalParameters) +
                   sizeof(STORAGE_PROTOCOL_SPECIFIC_DATA) + sizeof(uint64_t);
    ...
    if (iResult == 0) {
        ...
        if ((protocolDataDescr->Version !=
            ...
            (protocolDataDescr->Size !=
             sizeof(STORAGE_PROTOCOL_DATA_DESCRIPTOR))) {
            fprintf(stderr,
                    "[E] NVMeGetFeaturesTimestamp: Data descriptor header not "
                    "valid.\n");
            iResult = -1;  // error
        } else {
...
static void s_vNVMePrintTimestamp(uint64_t _ullData) {
    ...
    if (uAttributes & 0x1) {
        printf(
            "\t\tbyte [      6] bit [      0] 1 = The controller may have "
            "stopped counting\n");
    } else {
        printf(
            "\t\tbyte [      6] bit [      0] 0 = The controller counted time "
            "continuously\n");
    }
    ...
    if (uOrigin == 0) {
        printf(
            "\t\tbyte [      6] bit [  3:  1] 0 = The Timestamp field was "
            "initialzed to `0' by a Controller Level Reset\n");
    } else if (uOrigin == 1) {
        printf(
            "\t\tbyte [      6] bit [  3:  1] 1 = The Timestamp field was "
            "initialzed with a Timestamp value using a Set Features command\n");
    } else {
...
int iNVMeGetFeaturesTimestamp(HANDLE _hDevice) {
    ...
    iResult = iNVMeGetFeature32(_hDevice, NVME_FEATURE_TIMESTAMP,
