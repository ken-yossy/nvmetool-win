...
static void s_vPrintNVMeSupportedLogPages(PNVME_LID_SUPPORTED_AND_EFFECTS_DATA _pData)
{
    ...
    for (int i = 0; i < NUM_LIDS; i++)
    {
        ...
        if (_pData[i].LSUPP)
        {
            ...
            {
                printf("[I]     bit [    1] 0 = Index Offset is not supported\n");
            }
            ...
            if (i == 14) // 0Dh
            {
                ...
                {
                    printf("[I]     bit [   16] 1 = Establish Context and Read 512 Bytes of Header action is supported\n");
                }
                else
                {
                    printf("[I]     bit [   16] 0 = Establish Context and Read 512 Bytes of Header action is not supported\n");
                }
            }
            else if (0xC0 <= i)
            {
                printf("[I]     bit [31:16] 0x%u = LID specific field (vendor specific)\n", _pData[i].LIDSpecific);
            }
        }
        else
        {
...
int iNVMeGetSupportedLogPages(HANDLE _hDevice)
{
    ...
    protocolData->ProtocolDataRequestValue = NVME_LOG_PAGE_SUPPORTED_LOG_PAGES; // 0h
    ...
    s_vPrintNVMeSupportedLogPages((PNVME_LID_SUPPORTED_AND_EFFECTS_DATA)((PCHAR)protocolData + protocolData->ProtocolDataOffset));
    iResult = 0; // succeeded;
