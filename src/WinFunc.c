...
void vPrintSystemError(unsigned long _ulErrorCode, const char* _strFunc) {
    ...
    count = FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, _ulErrorCode,
                          0x0409,  // default
                          (LPTSTR)&lpMsgBuf, sizeof(lpMsgBuf), NULL);

    if (count != 0) {
        fprintf(stderr, "[E] %s: (error code = %d) %ls\n", _strFunc,
                _ulErrorCode, lpMsgBuf);
    } else {
...
int iIssueDeviceIoControl(HANDLE _hDevice, DWORD _dwControlCode,
                          LPVOID _lpInBuffer, DWORD _nInBufferSize,
                          LPVOID _lpOutBuffer, DWORD _nOutBufferSize,
    ...
    iRetIoControl = DeviceIoControl(
        _hDevice, _dwControlCode, _lpInBuffer, _nInBufferSize, _lpOutBuffer,
...
HANDLE hIssueCreateFile(const char* _strDeviceNo) {
    ...
    _stprintf_s(strDeviceName, 256, _T("\\\\.\\PhysicalDrive%hs"),
                _strDeviceNo);
    hDevice = CreateFile((LPCWSTR)strDeviceName, GENERIC_READ | GENERIC_WRITE,
                         FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
                         OPEN_EXISTING, FILE_ATTRIBUTE_DEVICE, NULL);
...
bool bCanUseGetDeviceInternalLog(void) {
    // if osInfo.dwBuildNumber < 19041, it's before Windows 10 May 2020 Update
    // (2004)
...
void vGetOSVersion(void) {
    ...
    PRtlGetVersion fpGetOSVer =
        (PRtlGetVersion)GetProcAddress(mod, "RtlGetVersion");
    if (fpGetOSVer == NULL) {
         ...
         VER_NT_WORKSTATION))  // not Windows Server 2016/2019
    {
        ...
        printf("[I] Running on Windows %d build %d", s_dwOSMajorVersion,
               s_dwOSBuildNumber);
    }
}
