    ...
    PNVME_AUTO_POWER_STATE_TRANSITION_ENTRY _entry)
{
    ...
    protocolData->ProtocolDataLength            = sizeof(NVME_AUTO_POWER_STATE_TRANSITION_ENTRY);
    ...
    if (iResult == 0)
    {
        ...
        {
            _result->AsUlong    = (ULONG)(protocolDataDescr->ProtocolSpecificData.FixedProtocolReturnData);
            resultData          = (PNVME_AUTO_POWER_STATE_TRANSITION_ENTRY)((PCHAR)protocolData + protocolData->ProtocolDataOffset);
            _entry->IdleTimePriorToTransition   = resultData->IdleTimePriorToTransition;
            _entry->IdleTransitionPowerState    = resultData->IdleTransitionPowerState;
        }
    }
...
int iNVMeGetFeaturesAPST(HANDLE _hDevice)
{
    ...
    NVME_CDW11_FEATURE_AUTO_POWER_STATE_TRANSITION CurrentSetting, DefaultSetting, SavedSetting;
    NVME_AUTO_POWER_STATE_TRANSITION_ENTRY CurrentEntry, DefaultEntry, SavedEntry;
    ...
    iResult = siNVMeGetFeaturesAPST(_hDevice, cdw10, &CurrentSetting, &CurrentEntry);
    if (iResult != 0) return iResult;

    // 2. Get default value
    cdw10.SEL = NVME_FEATURE_VALUE_DEFAULT;
    iResult = siNVMeGetFeaturesAPST(_hDevice, cdw10, &DefaultSetting, &DefaultEntry);
    if (iResult != 0) return iResult;

    // 3. Get saved value
    cdw10.SEL = NVME_FEATURE_VALUE_SAVED;
    iResult = siNVMeGetFeaturesAPST(_hDevice, cdw10, &SavedSetting, &SavedEntry);
    if (iResult != 0) return iResult;

    // 4. get supported capabilities
    iResult = iNVMeGetFeature32(_hDevice, NVME_FEATURE_AUTONOMOUS_POWER_STATE_TRANSITION, NVME_FEATURE_VALUE_SUPPORTED_CAPABILITIES, 0, (uint32_t*)&Cap);
    if (iResult != 0) return iResult;
    ...
    printf("\tIdle Time Prior to Transition (ITPT):\n"
        "\t\tCurrent = %d (ms)\n"
        "\t\tDefault = %d (ms)\n"
        "\t\tSaved   = %d (ms)\n",
