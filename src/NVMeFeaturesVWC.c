    ...
    if (iResult == 0) {
        ...
        if ((protocolDataDescr->Version !=
            ...
            (protocolDataDescr->Size !=
             sizeof(STORAGE_PROTOCOL_DATA_DESCRIPTOR))) {
            fprintf(stderr,
                    "[E] NVMeSetFeature: Data descriptor header not valid.\n");
            iResult = -1;  // error
        }
    }
    ...
    switch (iValue) {
        case 0:
            cCmd = cGetConsoleInput(
                "\n# Set Feature - Volatile Write Cache: disable and not save, "
                "Press 'y' to continue\n",
                strCmd);
            if (cCmd == 'y') {
                cdw11.WCE = 0;
        ...
        case 1:
            cCmd = cGetConsoleInput(
                "\n# Set Feature - Volatile Write Cache: enable and not save, "
                "Press 'y' to continue\n",
                strCmd);
            if (cCmd == 'y') {
                cdw11.WCE = 1;
        ...
        case 2:
            cCmd = cGetConsoleInput(
                "\n# Set Feature - Volatile Write Cache: disable and save, "
                "Press 'y' to continue\n",
                strCmd);
            if (cCmd == 'y') {
                cdw11.WCE = 0;
        ...
        case 3:
            cCmd = cGetConsoleInput(
                "\n# Set Feature - Volatile Write Cache: enable and save, "
                "Press 'y' to continue\n",
                strCmd);
            if (cCmd == 'y') {
                cdw11.WCE = 1;
