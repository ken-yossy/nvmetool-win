...
char* strConvertUTF8toMultiByte(const char* _str) {
    ...
    // Note that, numChar includes a terminating NULL character, because we
    // specified -1 to the parameter cbMultiByte
    ...
    if (MultiByteToWideChar(CP_UTF8, 0, _str, -1, (LPWSTR)strTemp, numChar) ==
        0) {
        vPrintSystemError(GetLastError(), "MultiByteToWideChar");
        goto error_exit;
    }
    ...
    // see
    // https://docs.microsoft.com/ja-jp/windows/desktop/api/stringapiset/nf-stringapiset-widechartomultibyte
    numChar = WideCharToMultiByte(CP_ACP, 0, (LPCWSTR)strTemp, -1, NULL, 0,
                                  NULL, NULL);
    if (numChar == 0) {
    ...
    if (WideCharToMultiByte(CP_ACP, 0, (LPCWSTR)strTemp, -1, (LPSTR)strDest,
                            numChar, NULL, NULL) == 0) {
        vPrintSystemError(GetLastError(), "WideCharToMultiByte");
    }
...
int eGetCommandFromConsole(void) {
    ...
    switch (cCmd) {
        ...
        case 'Z':
            printf(
                "\n# Format NVM : CAUTION!! all data are erased on the target "
                "drive");
        ...
        case 'X':
            cCmd = cGetConsoleInput(
                "\n# [EXPERIMENTAL] Vendor Specific Command (Admin FFh), Press "
                "'y' to continue\n",
                strCmd);
            if (cCmd == 'y') {
