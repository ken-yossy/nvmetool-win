...
static void s_vPrintNVMeTelemetryHostInitiated(
    PNVME_TELEMETRY_HOST_INITIATED_LOG _pData) {
    ...
    printf("\tbyte [  7:  5] %02X%02X%02X = IEEE OUI Identifier\n",
           _pData->OrganizationID[0], _pData->OrganizationID[1],
           _pData->OrganizationID[2]);
    printf(
        "\tbyte [  9:  8] %u = Telemetry Host-Initiated Data Area 1 Last "
        "Block\n",
        _pData->Area1LastBlock);
    printf(
        "\tbyte [ 11: 10] %u = Telemetry Host-Initiated Data Area 2 Last "
        "Block\n",
        _pData->Area2LastBlock);
    printf(
        "\tbyte [ 13: 12] %u = Telemetry Host-Initiated Data Area 3 Last "
        "Block\n",
        _pData->Area3LastBlock);

    if (_pData->ControllerInitiatedDataAvailable) {
        printf(
            "\tbyte [    382] %u = Telemetry Controller-Initiated Data is "
            "available\n",
            _pData->ControllerInitiatedDataAvailable);
    } else {
        printf(
            "\tbyte [    382] %u = Telemetry Controller-Initiated Data is not "
            "available\n",
            _pData->ControllerInitiatedDataAvailable);
    }

    printf(
        "\tbyte [    383] %u = Telemetry Controller-Initiated Data Generation "
        "Number\n",
...
static void s_vPrintNVMeTelemetryControllerInitiated(
    PNVME_TELEMETRY_HOST_INITIATED_LOG _pData) {
    ...
    printf("\tbyte [  7:  5] %02X%02X%02X = IEEE OUI Identifier\n",
           ...
           _pData->OrganizationID[2]);
    printf(
        "\tbyte [  9:  8] %u = Telemetry Controller-Initiated Data Area 1 Last "
        "Block\n",
        _pData->Area1LastBlock);
    printf(
        "\tbyte [ 11: 10] %u = Telemetry Controller-Initiated Data Area 2 Last "
        "Block\n",
        _pData->Area2LastBlock);
    printf(
        "\tbyte [ 13: 12] %u = Telemetry Controller-Initiated Data Area 3 Last "
        "Block\n",
        _pData->Area3LastBlock);
...
typedef union MY_STORAGE_PROTOCOL_DATA_SUBVALUE_GET_LOG_PAGE {
    ...
    ULONG AsUlong;
} MY_STORAGE_PROTOCOL_DATA_SUBVALUE_GET_LOG_PAGE,
    *MY_PSTORAGE_PROTOCOL_DATA_SUBVALUE_GET_LOG_PAGE;
...
static int s_iNVMeGetTelemetryHostInitiated(HANDLE _hDevice, bool _bCreate) {
    ...
    cdw10.NUMDL = NVME_TELEMETRY_DATA_BLOCK_SIZE >>
                  2;  // 512 / 4 (in NVMe, DWORD is 4 byte)
    ...
    if ((protocolDataDescr->Version !=
        ...
        fprintf(stderr,
                "[E] NVMeGetTelemetryHostInitiated: Data descriptor header not "
                "valid.\n");
        iResult = -1;  // error
        goto error_exit;
    }
    ...
    if ((protocolData->ProtocolDataOffset <
        ...
        fprintf(stderr,
                "[E] NVMeGetTelemetryHostInitiated: ProtocolData Offset/Length "
                "not valid.\n");
        iResult = -1;  // error
        goto error_exit;
    }
    ...
    {
        PNVME_TELEMETRY_HOST_INITIATED_LOG aLog =
            (PNVME_TELEMETRY_HOST_INITIATED_LOG)((PCHAR)protocolData +
                                                 protocolData
                                                     ->ProtocolDataOffset);
...
int iNVMeGetTelemetryControllerInitiated(HANDLE _hDevice) {
    ...
    if ((protocolDataDescr->Version !=
        ...
        (protocolDataDescr->Size != sizeof(STORAGE_PROTOCOL_DATA_DESCRIPTOR))) {
        fprintf(stderr,
                "[E] NVMeGetTelemetryControllerInitiated: Data descriptor "
                "header not valid.\n");
        iResult = -1;  // error
        goto error_exit;
    }
    ...
    if ((protocolData->ProtocolDataOffset <
         sizeof(STORAGE_PROTOCOL_SPECIFIC_DATA)) ||
        (protocolData->ProtocolDataLength <
         sizeof(NVME_TELEMETRY_HOST_INITIATED_LOG))) {
        fprintf(stderr,
                "[E] NVMeGetTelemetryControllerInitiated: ProtocolData "
                "Offset/Length not valid.\n");
        iResult = -1;  // error
        goto error_exit;
    }
...
static void s_vPrintDeviceInternalStatusData(
    PDEVICE_INTERNAL_STATUS_DATA _pData) {
...
static int s_iNVMeGetTelemetryHostInitiatedWithDeviceInternalLog(
    HANDLE _hDevice) {
    int iResult = -1;
    ULONG returnedLength = 0;
    ...
    // Allocate buffer for use.
    outBufferLength = FIELD_OFFSET(DEVICE_INTERNAL_STATUS_DATA, StatusData) +
                      STATUS_DATA_HEADER_LENGTH;
    ...
    // from
    // https://docs.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddstor/ns-ntddstor-get_device_internal_status_data_request
    ...
    iResult = iIssueDeviceIoControl(
        _hDevice, IOCTL_STORAGE_GET_DEVICE_INTERNAL_LOG, &request,
        sizeof(GET_DEVICE_INTERNAL_STATUS_DATA_REQUEST), outBuffer,
        outBufferLength, &returnedLength, NULL);
...
int iNVMeGetTelemetryHostInitiated(HANDLE _hDevice, bool _bCreate) {
    ...
    if (_bCreate && bCanUseGetDeviceInternalLog()) {
        cOpt = cGetConsoleInput(
            "#\t[EXPERIMENTAL] Using IOCTL_STORAGE_GET_DEVICE_INTERNAL_LOG may "
            "be supported; if you want to use it, press 'y'\n",
            strCmd);
    }

    if (cOpt == 'y') {
        iResult =
            s_iNVMeGetTelemetryHostInitiatedWithDeviceInternalLog(_hDevice);
    } else {
